...
//! ## 坑点 (Pitfalls)
//!
//! - **操作 0x11(GPIO_OUT) 必须读-改-写**，全字节覆写会关掉背光 GPIO2
//! - **0x09(I2C_CFG) 必须写 0x00** 禁用空闲休眠，否则 PMIC 半小时后不通
//! - **ID=0xff** 是正常的（M5STACK 定制芯片固件），不是故障
//! - **GPIO 默认输出类型为开漏**，必须先设 0x13=0x00 为推挽才能正常输出
...
macro_rules! make_probe {
    ($name:ident, $i2c:ty) => {
        fn $name() -> Option<I2cDriver<'static>> {
            ...
            for (reg, val) in &[
                (0x07, 0x05), // 保持 GPIO0 + GPIO2（NeoPixel + 背光）
                (0x06, 0x17), // 保留全部默认：充电 + DCDC + LDO + LED
                (0x09, 0x00), // 禁用 I2C 空闲休眠（M5GFX 必做，否则 PMIC 休眠后会断连）
                (0x16, 0x00),
                (0x10, 0x04), // GPIO2=输出
                (0x13, 0x00), // 推挽输出
                (0x11, 0x04), // GPIO2=高（背光亮）
                (0x43, 0x1F),
                (0x45, 0x07),
            ] {
