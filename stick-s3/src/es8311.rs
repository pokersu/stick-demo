...
//! Speaker 模式（0xB5），Mic 模式（0xBA）分别配置。

const ES8311_ADDR: u8 = 0x18;

/// ES8311 初始化（Speaker + Mic 合并配置）
/// 注意：寄存器 0x01 Speaker=0xB5, Mic=0xBA — 差异见 M5Unified
const ES8311_INIT: &[(u8, u8)] = &[
    (0x00, 0x80), // RESET / CSM POWER ON
    (0x01, 0xBA), // CLOCK_MANAGER / MCLK=BCLK (Mic 时钟)
...
pub fn init_es8311<I2C: embedded_hal::i2c::I2c>(i2c: &mut I2C) -> Result<(), &'static str> {
    for &(reg, val) in ES8311_INIT {
        i2c.write(ES8311_ADDR, &[reg, val]).map_err(|_| "ES8311 I2C error")?;
    }
